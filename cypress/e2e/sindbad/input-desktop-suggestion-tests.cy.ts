describe('Cities search desktop tests', () => {
  ...
  it('should have correct starting values', () => {
	...
	function findAndSelectCityTo(cityName: string, cityCode: number){
		...
		cy.get('[data-cy="dataInputTo"]').find('[data-cy="inputData"]').type(cityName);
      ...
      if (index === 0) {
		...
		cy.wait(500);
		cy.get('[data-cy="dataInputTo"]').find('[data-cy="inputData"]').should('have.value', cityName);
	}
	...
	function findAndSelectCityFrom(cityName: string, cityCode: number){
      ...
      if (index === 0) {
		...
		cy.wait(500);
		cy.get('[data-cy="dataInputFrom"]').find('[data-cy="inputData"]').should('have.value', cityName);
	}
})